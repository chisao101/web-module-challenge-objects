...
function createMenuItem(foodName, foodPrice, foodCategory){
  let newMenuItem = {name: foodName, price: foodPrice, category: foodCategory };
...
export const burger = {
  ...
  discount: function(customerStatus){
    if(customerStatus === 'teacher' || customerStatus === 'student'){
      this.price = this.price - (this.price * .25);
      return this.price;
    }else if(customerStatus === 'public'){
...
*/
let newReview = {name: 'Mike', rating: 4.5, feedback: "Got drunk. Ate food. Groovy.",};
...
function getReviewByIndex(theArray, arrayIndex) {
  let returnValue = `${theArray[arrayIndex].name} gave the restaurant a ${theArray[arrayIndex].rating} star review, and their feedback was: ${theArray[arrayIndex].feedback}`;
  return returnValue;
}
...
function getLastReview(anArray) {
  let returnValue = `${anArray[anArray.length - 1].name} gave the restaurant a ${anArray[anArray.length - 1].rating} star review, and their feedback was: ${anArray[anArray.length - 1].feedback}`;
  return returnValue;
} 
...
*/
 ...
 function getReviewByRating(theArray, theRating) {
  //  let ratedArray = [];
  //  for(let key in theArray){
  //    if(theArray[key].includes(theRating)){
  //      ratedArray.push(this.key);
  //    }
  //  }
